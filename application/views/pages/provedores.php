...
$(document).ready(function(){
		...
		if (agrego=='true'){ 
			...
			for (i = 0; i < cambios.length; i++){
				...
				$.ajax({
				       ...
				       type: "POST",
				       url: "<?=base_url()?>index.php/provedores/updateCliente/"+cambios[i],
				       success: function(){
	...
	$('#eliminar').click(function(){
		$('input:checked').each(function() {
			...
			$.ajax({
			       type: "POST",
			       url: "<?=base_url()?>index.php/provedores/delCliente/"+elem
			});
		});
	...
	$('#buscar').click(function(){
		...
		nombre = nombre.replace(")", "");
		$aux.attr('action',"<?=base_url()?>index.php/provedores/index/"+nombre+"/"+cuil+"/"+numero);
		$aux.submit();
	});
...
<?php }?>
<?php echo form_open('provedores/addCliente'); ?>
<div class='row'>
